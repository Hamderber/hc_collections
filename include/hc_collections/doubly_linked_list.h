...
/// @brief A doubly linked list, which contains data and tracks the address of the next and previous element or NULL if the
/// node is at the tail or head as applicable. pData will be NULL only when being used as the list's sentinel node.
typedef struct HC_DoublyLinkedList_t
{
    struct HC_DoublyLinkedList_t *pPrev;
    void *pData;
    struct HC_DoublyLinkedList_t *pNext;
} HC_DoublyLinkedList_t;

/// @brief Creates a new doubly linked list sentinel node, which is the root used for tracking the linked list itself.
/// @return New HC_DoublyLinkedList_t sentinel node (root) [HEAP] or NULL if alloc fails.
...
/// @return New HC_DoublyLinkedList_t node [HEAP] or NULL if alloc fails or pData is NULL.
...
/// @param pNode Node that already exists in the linked list. NOT elligible to be a sentinel node. Use hc_DLL_nodeAdd for that.
...
/// @param pNode Node that already exists in the linked list. NOT elligible to be a sentinel node. Use hc_DLL_dataAdd for that.
/// @param pData The data that will be tracked inside the newly created node that will be added to the linked list.
/// @return Result of attempting to add the data inside a new node. True only when a valid node is added to a valid linked list.
