...
int main(void)
{
    ...
    HC_DoublyLinkedList_t *pLink = hc_DLL_nodeNew(&dummyData);
    hc_DLL_nodeAdd(&pSentinel, pLink);
    fails += ut_assert(hc_DLL_nodeDetatch(&pLink) == true, "DLL nodeDelink (good node)");
    fails += ut_assert(pSentinel->pNext->pNext == NULL, "DLL nodeDelink (verify updated)");
    fails += ut_assert(pLink->pPrev == NULL, "DLL nodeDelink (verify cleared pPrev)");
    fails += ut_assert(pLink->pNext == NULL, "DLL nodeDelink (verify cleared pNext)");
    ...
    fails += ut_assert(hc_DLL_nodeAdd(&pSentinel, pNode) == false, "DLL nodeAdd (good node duplicate)");
    fails += ut_assert(hc_DLL_nodeRemove(&pSentinel, pNode) == true, "DLL nodeRemove (good node duplicate)");
    fails += ut_assert(hc_DLL_nodeRemove(&pSentinel, pNode) == false, "DLL nodeRemove (good node duplicate)");
    ...
    expectedResult = 7;
    fails += ut_assert(sumValidation == expectedResult, "DLL nodeInsertAfter (sumValidation = 7 (pPrev))");
    // printf("%d\n%d\n%d\n(?) %d\n",
